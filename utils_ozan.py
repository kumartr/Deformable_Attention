...
class unetUp_cca(nn.Module):
    def __init__(self, in_size, out_size, num_classes,is_deconv):
        super(unetUp_cca, self).__init__()
        self.conv = unetConv2(in_size, out_size, False)
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size+num_classes, out_size, kernel_size=4, stride=2, padding=1)
        else:
            self.up = nn.UpsamplingBilinear2d(scale_factor=2)
    ...
    def forward(self, inputs1, inputs2, cca):
        center_cat = torch.cat([cca, inputs2], dim=1)
        outputs2 = self.up(center_cat)
        offset = outputs2.size()[2] - inputs1.size()[2]
        padding = 2 * [offset // 2, offset // 2]
        outputs1 = F.pad(inputs1, padding)
        return self.conv(torch.cat([outputs1, outputs2], 1))
